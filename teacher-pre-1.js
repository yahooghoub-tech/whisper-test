...
if(newDay===currentTeacherCallDay)return;
...
supabaseClient
.channel("teacher-pre-1-realtime")
.on(
"postgres_changes",
{
event:"INSERT",
...
if(!call)return;
if(call.class_name!=="پیش-1")return;
...
if(deletedCall.class_name!=="پیش-1")return;

resetStudentButton(deletedCall);
loadCalls();
})
.subscribe(status=>{
console.log("Realtime teacher pre-1 status:",status);
});

supabaseClient
.channel("teacher-pre-1-attendance-realtime")
.on(
"postgres_changes",
{
event:"INSERT",
...
.subscribe(status=>{
console.log("Realtime attendance pre-1 status:",status);
});