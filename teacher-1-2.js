...
const students = [
    ...
    {name:"رادوین برادری همپا",className:"اول-2"},
...
function resetStudentButton(call){
    ...
    const {data,error}=await supabaseClient.from("calls").select("*").eq("class_name","اول-2").eq("called_date",today).order("id",{ascending:true});
    if(error){
    ...
    const {data,error}=await supabaseClient.from("calls").select("*").eq("student_name",student.name).eq("class_name","اول-2").neq("status","ارسال شد").order("id",{ascending:false}).limit(1);
    if(error){
    console.error(error);
    return;
    }
    ...
    const {data,error}=await supabaseClient.from("attendance").select("*").eq("class_name","اول-2").eq("attendance_date",today).eq("status","غایب");
    if(error){
    ...
    supabaseClient.channel("teacher-1-2-realtime").on("postgres_changes",{event:"INSERT",schema:"public",table:"calls",filter:"class_name=eq.اول-2"},payload=>{
        ...
        loadCalls();
        }).on("postgres_changes",{event:"UPDATE",schema:"public",table:"calls",filter:"class_name=eq.اول-2"},payload=>{
        const call=payload.new;
        const oldCall=payload.old;
        if(!call)return;
        if(call.class_name!=="اول-2")return;
        if(call.called_date!==getToday()){
        ...
        if(deletedCall.class_name!=="اول-2")return;
        resetStudentButton(deletedCall);
        loadCalls();
        }).subscribe(status=>{
        console.log("Realtime teacher status:",status);
        });
        supabaseClient.channel("teacher-1-2-attendance-realtime").on("postgres_changes",{event:"INSERT",schema:"public",table:"attendance",filter:"class_name=eq.اول-2"},payload=>{
        console.log("🟢 وضعیت حضور و غیاب جدید:",payload.new);
        if(!payload.new)return;
        if(payload.new.attendance_date!==getDatabaseToday())return;
        loadAbsentStudents();
        }).on("postgres_changes",{event:"UPDATE",schema:"public",table:"attendance",filter:"class_name=eq.اول-2"},payload=>{
        console.log("🟡 وضعیت حضور و غیاب تغییر کرد:",payload.new);
        if(!payload.new)return;
        if(payload.new.attendance_date!==getToday())return;
        loadAbsentStudents();
        }).on("postgres_changes",{event:"DELETE",schema:"public",table:"attendance",filter:"class_name=eq.اول-2"},payload=>{
