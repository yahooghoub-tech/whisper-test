...
supabaseClient
.channel("teacher-pre-2-realtime")
.on(
"postgres_changes",
{
event:"INSERT",
...
if(!call)return;
if(call.class_name!=="پیش-2")return;
...
if(deletedCall.class_name!=="پیش-2")return;

resetStudentButton(deletedCall);
loadCalls();
})
.subscribe(status=>{
console.log("Realtime teacher pre-2 status:",status);
});

supabaseClient
.channel("teacher-pre-2-attendance-realtime")
.on(
"postgres_changes",
{
event:"INSERT",
...
.subscribe(status=>{
console.log("Realtime attendance pre-2 status:",status);
});