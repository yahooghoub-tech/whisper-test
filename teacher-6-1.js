...
function resetStudentButton(call){
    ...
    const {data,error}=await supabaseClient.from("calls").select("*").eq("class_name","ششم-1").eq("called_date",today).order("id",{ascending:true});
    if(error){
    ...
    const {data,error}=await supabaseClient.from("attendance").select("*").eq("class_name","ششم-1").eq("attendance_date",today).eq("status","غایب");
    if(error){
    ...
    supabaseClient.channel("teacher-6-1-realtime").on("postgres_changes",{event:"INSERT",schema:"public",table:"calls",filter:"class_name=eq.ششم-1"},payload=>{
        ...
        supabaseClient.channel("teacher-6-1-attendance-realtime").on("postgres_changes",{event:"INSERT",schema:"public",table:"attendance",filter:"class_name=eq.ششم-1"},payload=>{
        console.log("🟢 وضعیت حضور و غیاب جدید:",payload.new);
        if(!payload.new)return;
        if(payload.new.attendance_date!==getDatabaseToday())return;
        loadAbsentStudents();
        }).on("postgres_changes",{event:"UPDATE",schema:"public",table:"attendance",filter:"class_name=eq.ششم-1"},payload=>{
        console.log("🟡 وضعیت حضور و غیاب تغییر کرد:",payload.new);
        if(!payload.new)return;
        if(payload.new.attendance_date!==getToday())return;
        loadAbsentStudents();
        }).on("postgres_changes",{event:"DELETE",schema:"public",table:"attendance",filter:"class_name=eq.ششم-1"},payload=>{
